...
zeromodel=,     /* ZeroModel specification */
...
symbol=dot,     /* plotting symbol for residuals              */
   ...
   out=pp,/* graph name in graphics catalog             */
   gout=
);
...
%if %substr(&dist,1,3) = ZIP %then %do;
array ybin{&nres} _b1 - _b&nres;
array ynb{&nres} _n1 - _n&nres;
%end;
...
%end;
%else %if %substr(&dist,1,3) = ZIP %then %do;
			call ranbin(seed, 1, p_psi, ybin[i]);
			call ranpoi(seed,p_lamb,ynb[i]); 
			_y_[i]=0*ybin[i]+(1-ybin[i])*ynb[i];
%end;
end;
run;
...
ods listing;
%halfnorm(
   data=testp,    /* Name of input data set                     */
   y=yp,             /* Name of response variable                  */
   resp=,          /* Name of response variable                  */
   trials=,        /* Name of trials variable (dist=bin only)    */
model=x,         /* Model specification                        */
class=,         /* Names of class variables                   */
   dist=poisson,          /* Error distribution                         */
link=log,          /* Link function                              */
   offset=,        /* Offset variable(s)                         */
   mopt=,          /* other model options (e.g., NOINT)          */
   freq=,          /* Freq variable                              */
id=,            /* Name of observation ID variable (char)     */
out=ppp,      /* output data set                            */
label=top 5,    /* NONE|ALL|ABOVE|TOP n                       */
   seed=0,         /* Seed for simulated residuals               */
   res=stdresdev,  /* Type of residual to use: strdeschi/stdresdev */
   nres=19,        /* Number of simulations for envelope         */
...
model=x,         /* Model specification                        */
class=,         /* Names of class variables                   */
   dist=negbin,          /* Error distribution                         */
link=log,          /* Link function                              */
   offset=,        /* Offset variable(s)                         */
   mopt=,          /* other model options (e.g., NOINT)          */
   freq=,          /* Freq variable                              */
...
label=top 5,    /* NONE|ALL|ABOVE|TOP n                       */
   seed=0,         /* Seed for simulated residuals               */
   res=reschi,  /* Type of residual to use: strdeschi/stdresdev */
   nres=19,        /* Number of simulations for envelope         */
symbol=dot,     /* plotting symbol for residuals              */
   interp=none,    /* interpolation for residuals                */
   color=red,      /* color for residuals                        */
...
ods listing;
%halfnorm(
   data=testnb,    /* Name of input data set                     */
   y=ynb,             /* Name of response variable                  */
   resp=,          /* Name of response variable                  */
   trials=,        /* Name of trials variable (dist=bin only)    */
model=x,         /* Model specification                        */
class=,         /* Names of class variables                   */
