...
%if %length(&trials) = 0 %then %do;
	%put ERROR: No TRIAL variable has been specified.;
	%let abort=1;
	%goto done;
	%end;
	
proc nlmixed data=ZIBD;
	parms %coefval(Coef = coefg, val=gv ) %coefval(Coef = coefb, val=bv);
	gb = %coefvar(Coef = coefg , vars = g );
	p_0 = exp(gb)/(1 + exp(gb));
	bb = %coefvar(Coef = coefb , vars = b );
	p_1 = exp(bb)/(1 + exp(bb));
		if &resp = 0 then prob = p_0 + (1-p_0)*(1-p_1)**&trials;
		if &resp ne 0 then prob= (1-p_0)*p_1**&resp*(1-p_1)**(&trials-&resp)*gamma(&trials+1)/(gamma(&resp+1)*gamma(&trials-&resp+1));
		if &resp = 0 then ll = log(p_0 + (1-p_0)*(1-p_1)**&trials);
					 else ll = log((1-p_0)*p_1**&resp*(1-p_1)**(&trials-&resp)*gamma(&trials+1)/(gamma(&resp+1)*gamma(&trials-&resp+1)));
...
data final;
	...
	mu    = (1-p_psi)*p_lamb*&trials;
	var   = mu*(1+p_lamb*&trials*p_psi-p_lamb);
...
drop tValue Alpha DF Lower Probt StdErrPred Upper;
	...
	do i=1 to dim(ynb);
			...
			call ranbin(seed,&trials,p_lamb,_y_[i]);
...
proc nlmixed data=_obstat_;
	...
	bb = %coefvar(Coef = coefb , vars = b );
	p_1 = exp(bb)/(1 + exp(bb));
		if _w&i = 0 then prob = p_0 + (1-p_0)*(1-p_1)**&trials;
		if _w&i ne 0 then prob= (1-p_0)*p_1**_w&i*(1-p_1)**(&trials-_w&i)*gamma(&trials+1)/(gamma(_w&i+1)*gamma(&trials-_w&i+1));
		if _w&i = 0 then ll = log(p_0 + (1-p_0)*(1-p_1)**&trials);
					else ll= log((1-p_0)*p_1**_w&i*(1-p_1)**(&trials-_w&i)*gamma(&trials+1)/(gamma(_w&i+1)*gamma(&trials-_w&i+1)));
...
data final&i;
	merge lamb&i(rename=(Pred=p_lamb&i)) psi&i(rename=(pred=p_psi&i));
		by obsnumber;
	mu&i = (1-p_psi&i)*p_lamb&i*&trials;
	var&i = mu&i*(1+p_lamb&i*&trials*p_psi&i-p_lamb&i);
...
proc datasets nofs nolist nowarn library=work memtype=(data);
	delete _obstat_ ZIBD psi final lamb _sorted_
		%do i=1 %to &nres;
		...
		run; quit;
%done:
%if &abort %then %put ERROR: The HALFNORM_ZIB macro ended abnormally.;
%mend halfnorm_zib;
...
p_1 = exp(eta_lambda) / (1 + exp(eta_lambda));
						call ranbin(seed,m,p_1,ynb1);
				yzib=0*zero_inflate+(1-zero_inflate)*ynb1;
				output;
		end;
end;
keep m yzib x ;
...
/*********************************************************************************************************************************/
/***************************************		HalfNormal ZINegative Binomial			******************************************/
/*********************************************************************************************************************************/
...
proc nlmixed data=ZIBD;
	...
	bb = %coefvar(Coef = coefb , vars = b );
	p_1 = exp(bb);
		if &resp = 0 then prob = p_0 + (1-p_0)*(1+&trials*p_1)**(-1/&trials);
		if &resp ne 0 then prob= (1-p_0)*((gamma(&resp+1/&trials)*(&trials*p_1)**&resp)/(gamma(&resp+1)*gamma(1/&trials)*(1+&trials*p_1)**(&resp+1/&trials)));
		if &resp = 0 then ll = log(p_0 + (1-p_0)*(1+&trials*p_1)**(-1/&trials));
					 else ll = log((1-p_0)*((gamma(&resp+1/&trials)*(&trials*p_1)**&resp)/(gamma(&resp+1)*gamma(1/&trials)*(1+&trials*p_1)**(&resp+1/&trials))));
...
data final;
	merge lamb(rename=(Pred=p_lamb)) psi(rename=(pred=p_psi)); by obsnumber;
	mu    = (1-p_psi)*p_lamb;
	var   = mu+((p_psi/(1-p_psi))+(&trials/(1-p_psi)))*mu**2;
	odres = abs((&resp-mu)/sqrt(var));
...
drop tValue Alpha DF Lower Probt StdErrPred Upper;
	...
	do i=1 to dim(ynb);
			call ranbin(seed, 1, p_psi, ybin[i]);
			call rangam(seed,1/&trials,mean);
			call ranpoi(seed,mean*p_lamb*&trials,_y_[i]);
					ynb[i]=0*ybin[i]+(1-ybin[i])*_y_[i];
	end;
run;
...
proc nlmixed data=_obstat_;
	...
	bb = %coefvar(Coef = coefb , vars = b );
	p_1 = exp(bb);
		if _w&i = 0 then prob = p_0 + (1-p_0)*(1+&trials*p_1)**(-1/&trials);
		if _w&i ne 0 then prob= (1-p_0)*((gamma(_w&i+1/&trials)*(&trials*p_1)**_w&i)/(gamma(_w&i+1)*gamma(1/&trials)*(1+&trials*p_1)**(_w&i+1/&trials)));
		if _w&i = 0 then ll = log(p_0 + (1-p_0)*(1+&trials*p_1)**(-1/&trials));
					else ll = log((1-p_0)*((gamma(_w&i+1/&trials)*(&trials*p_1)**_w&i)/(gamma(_w&i+1)*gamma(1/&trials)*(1+&trials*p_1)**(_w&i+1/&trials))));
...
data final&i;
	...
	var&i = mu&i+((p_psi&i/(1-p_psi&i))+(&trials/(1-p_psi&i)))*mu&i**2; 
	odres&i = abs((_w&i-mu&i)/sqrt(var&i));
run;
...
eta_lambda = bll_0 + bll_1*x;
p_1 = exp(eta_lambda);
				call rangam(seed,1/m,mean);
				call ranpoi(seed,mean*p_1*m,ynb1);
